...
**Verification:** every headline number is independently recomputed from
the raw subsets by a clean-room reimplementation committed at
[analytics/verification/verify_headlines.py](../analytics/verification/verify_headlines.py)
— AAPL's time-weighted spread matches to full float precision
(1.3490716886 bps) and the spot OFI bucket exactly (−6,853 over 2,783
events, 10:00–10:01). Run it yourself.
